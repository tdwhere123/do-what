# do-what v0.1.x — Phase 总览

v0.1 主线（E0–E7，T001–T027，40 个 soul 测试）已全部交付。
v0.1.x 是收敛/清理/补魂阶段：不引入颠覆性架构变化，只做减法、补全和精化。
...
| ID | 名称 | 路线 | 状态 |
|----|------|------|------|
| T028 | adapter-layer-cleanup | A：接入层清理 | 待开始 |
| T029 | event-state-reduction | B：事件/状态减法 | 待开始 |
| T030 | doc-naming-cleanup | K：文档命名收边 | 待开始 |
...
| ID | 名称 | 路线 | 状态 |
|----|------|------|------|
| T031 | soul-concept-unification | H：概念减法+命名统一 | 待开始 |
| T032 | context-lens | 6.12 章 | 待开始 |
| T033 | claim-form-memory-slot | 6.4–6.5 章 | 待开始 |
| T034 | memory-dynamics | 6.9 章 | 待开始 |
| T035 | graph-recall-bounded | 6.8 章 | 待开始 |
| T036 | evidence-capsule | 7.1 章 | 待开始 |
| T037 | user-ledger | 7.2 章 | 待开始 |
...
| ID | 名称 | 路线 | 状态 |
|----|------|------|------|
| T038 | core-hot-state | C：控制态与 Projection 分离 | 待开始 |
| T039 | projection-layer | C（续）+ D：异步读模型 | 待开始 |
| T040 | ack-overlay-sync-async | C/D：Ack Overlay + 路径硬切分 | 待开始 |
| T041 | memory-repo-demotion | I：memory_repo 降格典藏层 | 待开始 |
...
| ID | 名称 | 路线 | 状态 |
|----|------|------|------|
| T042 | focus-surface-baseline-lock | F：精确漂移判定 | 待开始 |
| T043 | integration-gate | F（续）：三类漂移 + 防活锁 | 待开始 |
| T044 | governance-lease | G：预飞行治理 + 原生表面报告 | 待开始 |
| T045 | orchestration-template-constraints | E：编排减法 + 四种允许拓扑 | 待开始 |
