...
- 检查是否存在相同 `anchors` + `source` + `project_id` 的 cue（防重复）
...
- git commit message：`feat(memory): add <dimension> cue - <gist[:50]>`（dimension 可能为 NULL，此时用 'cue' 替代）
...
- 重复 cue 检测使用 `anchors` + `source` + `project_id` 的组合
...
PROPOSAL=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" \
  ...
  -d '{"tool":"soul.propose_memory_update","args":{"project_id":"test-proj","cue_draft":{"gist":"monorepo uses pnpm workspace","anchors":["monorepo"],"pointers":["git_commit:abc123 repo_path:package.json"]},"confidence":0.9,"impact_level":"canon"}}' \
...
- **风险：** 重复 cue 检测过于严格（相同 anchors+source 的合理重复内容被合并）
