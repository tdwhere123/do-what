...
- `MemorySearchService`：
  ...
  - 过滤：`project_id`、`tracks`（可选）、`impact_level >= working`；v0.1.x 阶段可选按 `scope`/`dimension`/`domain_tags` 过滤
...
INSERT INTO memory_cues (cue_id, project_id, gist, source, track, anchors, pointers, confidence, impact_level, created_at, updated_at)
VALUES (
  'cue-test-1', 'test-proj', 'authentication logic refactored to service layer',
  'compiler', 'architecture', '[\"auth\",\"service\"]', '[\"git_commit:abc123\"]',
  0.8, 'consolidated', datetime('now'), datetime('now')
);"
