...
**Context:** Soul 的存储基础。依赖 T007（迁移框架）。soul.db 独立于 state.db，有独立 Worker 线程。
...
- [ ] `migrations/v1.ts`：4 张表（memory_cues, memory_graph_edges, evidence_index, soul_schema_version）+ FTS5 虚拟表 + 3 个触发器（AFTER INSERT/DELETE/UPDATE on memory_cues）
- [ ] `soul-worker.ts`：独立 DatabaseWorker（结构同 Core 的 database-worker.ts，连接 soul.db）
- [ ] `soul-state-store.ts`：只读连接 stub（T018/T019 实现具体查询）
- [ ] `schema.ts`：`CueRow`, `EdgeRow`, `EvidenceRow` interface
...
- [ ] `memory-search.ts`：FTS5 查询（FTS5 不可用时 fallback LIKE）；project_id + track + archived=0 过滤；Bootstrapping Phase 处理（confidence>=0.6 的 Working 级加 `[trial]` 标注）；limit + budget 截断
...
**Exact tasks:**
- [ ] `cue-writer.ts`：upsert（同 project_id+anchors+type 的 cue update，否则 insert）；晋升逻辑（hit_count>=3 且 pointers 非空 → Consolidated）
