...
### 2.6 Follow-up Fixes (v0.10 回归补丁)
- `packages/app/src/app/pages/session.tsx`
  - 移除右上 `SessionDagWidget` 入口，彻底摘除“分支会话”展示链路。
- `packages/app/src/app/components/session-dag.tsx`
  - 文件删除（不再保留分支 DAG 逻辑）。
- `packages/app/src/app/state/sessions.ts`
  - 删除 `Project` 状态存储与 `createProject/addSessionToProject/useProjects`；
  - 删除 `AgentRun.projectId/parentSessionIds`；
  - 删除 `listAgentRunTextEvents`（不再做 parent session context 拼接）。
- `packages/app/src/app/lib/shared-config.ts`
  - 删除 `buildContextPrefix`（彻底移除 parent-session 上下文拼接逻辑）。
- `packages/app/src/app/components/do-what-logo.tsx`
  - 改为圆形 SVG Logo（替代旧图形资源引用）。
- `packages/desktop/src-tauri/src/workspace/files.rs`
  - 移除 enterprise skill 包下载逻辑；
  - 移除 starter/automation 默认插件 (`opencode-scheduler`) 与默认 MCP (`control-chrome`) 注入；
  - 保留最小初始化结构，外部插件/仓库由用户后续显式安装。
- `packages/desktop/src-tauri/src/commands/runtimes.rs`
  - runtime 版本探测补齐 Windows `.cmd/.bat` 执行路径（`cmd /C`）。
- `packages/app/scripts/_util.mjs`
  - UI 健康脚本在 Windows 下通过 `cmd /C opencode ...` 启动，避免直接 spawn shim 失败。
...
- 删除 `packages/app/src/app/components/session-dag.tsx`（分支会话 UI）。
- 删除 `state/sessions.ts` 中 project/parent-session 关联状态与写入逻辑。
- 删除 workspace 初始化默认插件与 enterprise skills 自动注入逻辑。
...
- Not passed in current environment:
  - `pnpm --filter @do-what/ui test:health`
  - `pnpm --filter @do-what/ui test:sessions`
  - 失败原因一致：本机环境未安装 `opencode`（stderr: `'opencode' is not recognized as an internal or external command`），非代码编译错误。
