...
- [x] `pnpm typecheck` 通过
- [x] Settings > Runtimes tab 存在，显示 Claude Code 和 Codex 状态
- [x] Composer 底部有运行时 dropdown
...
- 已完成：运行时检测命令、Runtimes 设置页、输出解析器、AgentRun 组件与 store 基础能力。
- 部分完成：Composer 仅完成运行时选择 UI，非 OpenCode 发送链路与 Sidebar 融合留在下一轮。
- 详细落地见：`plans/v0.2-recond.md`。
