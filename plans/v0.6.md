...
## 13. 实施记录（2026-02-28，Router 可选连接能力移除）

1. 主链路行为调整（由“可选启用”改为“主线移除”）：
- `packages/desktop/scripts/prepare-sidecar.mjs` 固定禁用 router sidecar，历史开关仅保留兼容读取并输出 ignored 提示。
- `packages/orchestrator/src/cli.ts` 固定 `opencodeRouterEnabled=false`，历史 CLI/env 开关不再生效并提示 ignored。
- `packages/desktop/src-tauri/build.rs` 不再参与 router sidecar 兜底装配。
- `packages/app/src/app/lib/tauri.ts` 移除前端 router 状态与控制调用封装，避免误接入。

2. 开发链路联动：
- `scripts/dev-headless-web.ts` 移除 router 启动参数与构建依赖，日志显式标记“router 已从主线移除”。
- `package.json` 根脚本 `test:orchestrator` 对齐为 orchestrator typecheck（不依赖 router 路径）。

3. 文档联动：
- 已同步：`README.md`、`packages/app/README.md`、`packages/desktop/README.md`、`packages/orchestrator/README.md`、`packages/server/README.md`。
- 本次补充：`packages/orchestrator/package.json` 描述移除 “optional opencode-router” 表述。

4. 稳定性检查（CLI）：
- 通过：`pnpm.cmd --filter @do-what/ui typecheck`
- 通过：`pnpm.cmd --filter @do-what/server typecheck`
- 通过：`pnpm.cmd --filter @do-what/orchestrator typecheck`
- 通过：`pnpm.cmd --filter @do-what/server test`
- 通过：`pnpm.cmd --filter @do-what/orchestrator build`
- 通过：`pnpm.cmd --filter @do-what/ui build`
- 通过：`pnpm.cmd --filter @do-what/desktop exec cargo check --manifest-path src-tauri/Cargo.toml`
- 受环境限制未通过：`pnpm.cmd --filter @do-what/ui test:health`、`pnpm.cmd --filter @do-what/ui test:sessions`（`spawn opencode ENOENT`，本机缺少 `opencode` 可执行文件）。

5. 删除动作：
- 删除清单：空
- 删除理由：本次为主链路收敛与风险规避，未执行物理删除。
- 验证命令：`git status --short`

## 14. 实施记录（2026-02-28，Router 代码路径摘除 + 稳定性复检）

1. 摘除范围（主线代码，不再是 ignored 开关）：
- `packages/orchestrator/src/cli.ts`：
  - 删除 `opencode-router` 二进制解析、健康检查、sandbox 注入、proxy 自检路径。
  - 删除 `OPENCODE_ROUTER_*` 环境变量透传。
  - CLI help 不再暴露 `--opencode-router*` 历史参数提示。
- `packages/orchestrator/src/tui/app.tsx`：
  - 删除 Router 视图、Router 快捷键、Router 配置/身份信息逻辑。
  - TUI 保留 `overview/logs/help` 三视图。
...
3. 删除理由：
- Router 连接链路是已退役的可选能力，继续保留会引入配置漂移与误接入风险。
- 按“主链路优先稳定”原则，直接摘除代码路径比“兼容 ignored”更安全可控。
...
5. 文档联动：
- 已更新：`README.md`、`packages/orchestrator/README.md`、`packages/desktop/README.md`、`packages/server/README.md`、`packages/app/README.md`
- `README: 已同步`
