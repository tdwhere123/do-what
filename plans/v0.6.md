...
## 10. 实施记录（2026-02-28，env 兼容迁移）

1. 配置读取层新增统一 env 兼容读取函数，支持 `DOWHAT_* > OPENWORK_*` 优先级。
2. server/orchestrator/desktop 的读取入口已切换到兼容读取。
3. 保留 `OPENWORK_*` 兼容并增加一次性 deprecated 提示，保证旧链路不阻塞。
4. 同步更新相关 README 的环境变量说明。
5. 新增执行记录：`plans/execution/02-env-compat-migration.md`。
