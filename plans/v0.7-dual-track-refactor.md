...
## Track 2 执行记录（2026-02-28）

- 已完成：Step 2（桌面端生命周期修复）
  - 应用退出时清理本地 runtime 子进程
  - 本地 runtime done 事件上报真实退出码
- 已完成：Step 3（多助手并行状态模型）
  - 新增 `check_assistant_statuses` 聚合接口
  - 新增 `check_opencode_status` / `check_claude_code_status` / `check_codex_status` 独立接口
  - 返回统一结构：Installed/Not Installed + Logged In/Logged Out + 版本 + 诊断信息
- 已完成：Step 4（Settings 控制器重构）
  - Settings Runtime 页改为调用统一模型并并列展示三助手状态
- 已完成：Step 1（解耦）
  - `DOWHAT_*` 前缀在 desktop/orchestrator 关键启动路径优先生效，并保留 `OPENWORK_*` 兼容
  - 顶层/包级元信息（desktop/orchestrator/server）完成 do-what 对齐
  - `router` 连接链路已从主线代码路径摘除（非 ignored 逻辑）：
    - orchestrator CLI 删除 router binary/health/sandbox/proxy 检查残留
    - orchestrator TUI 删除 Router 视图与交互
    - 删除 `packages/orchestrator/scripts/build-opencode-router.mjs` 与 `packages/orchestrator/scripts/router.mjs`
  - 稳定性复检：
