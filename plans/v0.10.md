...
## Implementation Status (Follow-up, 2026-03-01)
- 已按问题回归补齐以下项：
  - 移除会话页右上 `SessionDagWidget` 与 project/parent-session 分支链路展示；
  - Composer/发送链路改为纯 do-what 主链（不再注入 parentSession context，不再写 project 关联）；
  - OpenAI OAuth 增强回退（method index 多候选尝试 + Modal 手动完成按钮流）；
  - 运行时平权修复：`opencode/claude-code/codex` 在 UI 连接判定上统一为“已安装可用”，Windows 下 runtime 探测支持 `.cmd/.bat`；
  - 清空默认内置技能/插件注入：移除 workspace 初始化阶段的 enterprise skills 下载、默认 `opencode-scheduler` 与 `control-chrome` 注入；
  - 桌面/前端 logo 改为圆形风格（Tauri 图标已重生成为圆形，UI `DoWhatLogo` 组件改为圆形 SVG）。
