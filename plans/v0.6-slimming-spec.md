...
- 现状：负责拉起 opencode + openwork-server，`router` 连接链路已从主线代码路径摘除，文件体量大（`src/cli.ts`）。
- 结论：保留“业务编排最小集”，router 相关路径不再纳入主线能力；TUI/高级诊断逻辑继续模块化收敛。
...
范围：
1. `orchestrator` 保持 router 主线禁用，并把 TUI/高级诊断逻辑改为按需加载。
2. `server` 关闭 toy-ui 默认暴露，扩展接口放到可选注册段。
3. 统一环境开关命名与默认值（业务优先，不再暴露 router 连接能力开关）。
...
4. 本次补充（2026-02-28）：
- 已完成 router 主线代码路径摘除（orchestrator CLI + TUI + sidecar 脚本）。
- 删除清单：
