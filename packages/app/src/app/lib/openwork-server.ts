...
export function createOpenworkServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    activateWorkspace: (workspaceId: string) =>
      requestJson<{ ok: boolean; activeWorkspaceId: string }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/activate`, {
        token,
        hostToken,
        method: "POST",
    ...
    getConfig: (workspaceId: string) =>
      requestJson<{ opencode?: Record<string, unknown>; openwork?: Record<string, unknown> }>(
        baseUrl,
