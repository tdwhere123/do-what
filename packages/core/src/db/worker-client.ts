...
export class WorkerClient {
  ...
  async write(request: DbWriteRequest): Promise<void> {
    if (this.closed) {
      throw new Error('worker client is closed');
    }
  ...
  private spawnWorker(): Worker {
    ...
    worker.on('message', (message: WorkerResultMessage) => {
      ...
      pendingRequest.reject(new Error(message.error ?? 'database write failed'));
    });
    ...
    worker.on('exit', (code) => {
      if (code !== 0) {
        this.tryRestart(new Error(`database worker exited with code ${code}`));
      }
    });
  ...
  private tryRestart(error: Error): void {
    ...
    if (this.restartAttempts >= MAX_RESTART_ATTEMPTS) {
      this.failAllPending(
        new Error(
          `database worker restart limit reached: ${error.message}`,
        ),
      );
      return;
    }
