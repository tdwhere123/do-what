...
CREATE TABLE memory_cues (
  ...
  gist          TEXT NOT NULL,           -- 短摘要（可注入）
  type          TEXT NOT NULL,           -- 'fact|pattern|decision|risk'
  ...
  source        TEXT NOT NULL,          -- 'compiler|user|seed'
  ...
  ttl_days      INTEGER,
  metadata      TEXT                    -- JSON blob（扩展字段）
);
CREATE INDEX idx_cues_project ON memory_cues(project_id, impact_level, archived);
...
CREATE TABLE memory_graph_edges (
  ...
  relation    TEXT NOT NULL,  -- 'implements|depends_on|contradicts|extends|replaces'
  track       TEXT,
  confidence  REAL NOT NULL DEFAULT 0.5,
...
2. 创建 `src/db/schema.ts`：`CueRow`, `EdgeRow`, `EvidenceRow` interface（对应 SQL 列）
3. 创建 `src/db/migrations/v1.ts`：完整 DDL + FTS5 + 触发器
...
INSERT INTO memory_cues (cue_id, project_id, gist, type, anchors, pointers, created_at, updated_at)
VALUES ('test-1', 'proj-1', 'auth logic moved to middleware', 'pattern', '[\"auth\"]', '[]', datetime('now'), datetime('now'));
